...
\renewcommand{\thesisauthor}{Seán Joseph Lawlor}                %% Your official TCD name.
\renewcommand{\thesismonth}{May}                   %% Your month of graduation.
\renewcommand{\thesisyear}{2026}                      %% Your year of graduation.
\renewcommand{\thesistitle}{Develop a lighttpd test setup for ECH}          %% The title of your thesis; use mixed-case.
...
\renewcommand{\thesisdegree}{BA(Mod) in Science in Computer Science}
...
\renewcommand{\thesisdegreeabbreviation}{BA(Mod}
...
\begin{titlepage}
\centering
\includegraphics[width=1\textwidth]{TrinityCrest.png}\\[2cm] % adjust size or spacing
\thesistitlepage
\end{titlepage}
...
This project explores the development of a local test environment for evaluating
Encrypted Client Hello (ECH) using the Lighttpd web server. ECH, an extension to
TLS~1.3, encrypts sensitive information in the ClientHello message to improve user
privacy. The goal of this report is to build, configure, and test a working
Lighttpd installation capable of performing ECH handshakes using BoringSSL and to
verify its functionality through practical experimentation.
The work includes compiling Lighttpd with ECH-capable TLS libraries, generating
ECH configuration files, and validating results with tools such as \texttt{bssl client},
\texttt{curl}, Wireshark, and modern browsers. The outcome demonstrates the
feasibility of integrating ECH within lightweight web server environments and
documents configuration issues, debugging approaches, and performance observations.
\end{thesisabstract}
